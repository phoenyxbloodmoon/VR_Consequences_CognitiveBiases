...
#wilcox.test(accuracy ~ TA_cat, data = anxiety_1) #0.02229
...
wilcox.test(accuracy ~ group, data = dataset_gamblers) #0.9163
...
wilcox.test(accuracy ~ group, data = dataset_crt) #0.4287
...
wilcox.test( reaction_time ~ group, data = dataset_catch)

t.test( accuracy ~ group, data = dataset_catch)
wilcox.test(accuracy ~ group, data = dataset_catch)
#----------------
wilcox_anova <- data_1 %>%
